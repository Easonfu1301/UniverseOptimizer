...
class LearningAgent(BaseAgent):
    ...
    def learn(self):
        prompt = f"""
你将看到一个算法，由{self.script_path}所运行，它的默认配置是{self.default_config}，它的优化指标是{self.metrics_to_optimize}。
你需要根据这些信息，你需要做以下事情：
1. 了解最终的优化目标是如何定义和计算的。
2. 了解定义的参数({self.default_config.keys()})是如何影响优化目标的，为每一个参数生成一个总结
3. 一步步的描述算法的运行过程
最后，你需要在{self.workdir}中生成一个几个md文件，分别是：
1. 一个总结文件，包含算法的运行过程和优化目标的定义和计算方法，文件名为logic.md
2. 一个参数总结文件，包含每一个参数的总结，文件名为params.md，每一个参数为一个小节，包含参数的作用，参数的默认值，参数的取值范围，参数的影响因素，参数的优化方法等
        """
